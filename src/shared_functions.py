...
def load_food_data(file_path: str) -> List[Dict]:
    ...
    try:
        ...
        print(f"Successfully loaded {len(food_data)}")
        return food_data
...
def create_similarity_search_collection(collection_name: str,
    ...
    """Create ChromaDB collection with sentence transformer embedding"""
    try:
...
def populate_similarity_collection(collection,food_items: List[Dict]):
    """Populate the collection with food data and generate embeddings"""
    ...
    for i,food in enumerate(food_items):
        # Create a comprehensice text for embedding using rich JSON structure
        text = f"Name: {food['food_name']}."
        text += f"Description: {food.get("food_description"," ")}."
        text += f"Ingredients: {" ,".join(food.get("food_instructions",[]))}."
...
def perform_similarity_search(collection,query: str,n_results: int = 5):
    """Perform similarity search and return the formatted results"""
    try: 
        ...
        for i in range(len(results['ids'][0])):
            ...
            result = {
                ...
                "food_name":results["metadatas"][0]['name'],
        ...
        print(f"Error found in similarity search: {e}")
        return []
